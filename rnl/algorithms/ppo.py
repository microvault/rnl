...
class PPO:
    ...
    def __init__(
        ...
        self.env_navigation = env_navigation
    ...
    def get_action(self, state, action=None, grad=False, action_mask=None):
        ...
        if self.discrete_actions:
            ...
            if torch.isnan(action_probs).all():
                print(
                    "Todas as probabilidades são NaN. Escolhendo ação aleatoriamente."
                )
                # Determina o número de ações
    ...
    def test_with_animation(self):
        ...
        with torch.no_grad():
            state, info = self.env_navigation.reset()
            done = False
            i = 0
            while not done:
                action, _, _, _ = self.get_action(state)
                done = self.env_navigation.step_animation(i=i, action=action, test=True)
                i += 1
        ...
        return wandb.Video(output_file, fps=30)
