...
# Função 2: Recompensa de orientação (quanto menor o alpha, melhor)
@njit
...
@njit
def normalized_progress_reward(
    ...
    """
    Calcula uma recompensa normalizada baseada na aproximação ao objetivo,
    com um fator de escala ajustável.

    Args:
        distance_initial: Distância inicial do robô ao objetivo
        current_distance: Distância atual do robô ao objetivo
        min_distance: Distância mínima possível ao objetivo
        max_distance: Distância máxima possível ao objetivo
        scale_factor: Fator de escala para ajustar a magnitude da recompensa (padrão: 1.0)

    Returns:
        Recompensa escalada entre -scale_factor e scale_factor
    """
    # Calcular a diferença (progresso)
    diff = distance_initial - current_distance

    # Calcular o progresso máximo possível (em ambas as direções)
    max_progress = max_distance - min_distance

    # Normalizar a recompensa para o intervalo [-1, 1]
    normalized_reward = diff / max_progress

    # Aplicar o fator de escala
    scaled_reward = normalized_reward * scale_factor
