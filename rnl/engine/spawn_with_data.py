...
def _point_in_ring(px, py, ring):
    ...
    de um anel (lista de coordenadas [(x0, y0), (x1, y1), ...]).
    """
    inside = False
    n = len(ring)
    for i in range(n):
...
def _point_in_polygon(px, py, exterior, holes):
    """
    Verifica se o ponto (px, py) está dentro do polígono definido pelo exterior e buracos.
    """
...
def spawn_robot_and_goal_with_data(poly, robot_clearance=3.0, goal_clearance=3.0, min_robot_goal_dist=2.0, max_tries=1000):
    """
    Gera posições aleatórias para o robô e o objetivo dentro do polígono 'poly',
    considerando clearances e uma distância mínima entre eles.
    """
    ...
    if safe_poly_robot.is_empty or safe_poly_goal.is_empty:
        raise ValueError("Clearance muito grande. Polígono inválido.")
    ...
    for _ in range(max_tries):
        robo_pos = random_point_in_poly((minx_r, miny_r, maxx_r, maxy_r), ext_robot, holes_robot)
        goal_pos = random_point_in_poly((minx_g, miny_g, maxx_g, maxy_g), ext_goal, holes_goal)
