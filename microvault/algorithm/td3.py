...
class TD3Agent:
    ...
    def __init__(self, state_size, action_size, random_seed, CONFIG_PATH):
        ...
        self.actor_optimizer = optim.Adam(
            self.actor_local.parameters(), lr=self.param["LR_ACTOR"]
        )
        ...
        self.critic_optimizer = optim.Adam(
            self.critic_local.parameters(), lr=self.param["LR_CRITIC"]
        )
    ...
    def learn(
        ...
        if len(self.memory) > self.batch_size:
            for i in range(n_iteraion):
                ...
                # Generate a random noise
                noise = (
                    torch.FloatTensor(action_).data.normal_(0, self.noise).to(device)
                )
                noise = noise.clamp(-self.noise_clip, self.noise_clip)
                actions_next = (actions_next + noise).clamp(
                    -1.0, 1
                )  # mudar aqui depois
