...
for j, (norm, raw, h, lbl, col) in enumerate(
    zip(
        [com_norm, sem_norm],
        [com_raw, sem_raw],
        hatch,
    ...
    pos = x + (j - 0.5) * bw
    bars = ax.bar(
        pos, norm, bw, color=col, hatch=h, edgecolor="black", linewidth=0.8, label=lbl
    )
...
ax.set_xticks(x)
ax.set_xticklabels(
    [f"{m}\n(min=0, max={mx})" for m, mx in zip(metrics, ranges_max)], fontsize=9
)
...
ax.legend(
    bbox_to_anchor=(0.5, 1.06), loc="lower center", ncol=2, frameon=False, fontsize=9
)
